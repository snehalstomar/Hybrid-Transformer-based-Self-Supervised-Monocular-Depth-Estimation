...
class convprojection_base(nn.Module):
    def __init__(self, path=None, **kwargs):
        ...
        self.conv1 = nn.Conv2d(8,1,3,1,1)
...
#         res16x = res32x + x1[3]
        ...
        # exit()
        op[("disp", 0)] = self.sig(self.conv1(x))
        op[("disp", 1)] = self.sig(self.conv2(res2x))
        op[("disp", 2)] = self.sig(self.conv3(res4x))
        op[("disp", 3)] = self.sig(self.conv4(res8x))
