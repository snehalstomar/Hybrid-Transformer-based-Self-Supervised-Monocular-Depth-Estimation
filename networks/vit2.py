...
class Depth_Block(nn.Module):
    def __init__(self, n_feat):
        ...
        self.block = [CAB(n_feat) for _ in range(2)]

        self.block = nn.Sequential(*self.block)
        self.conv1 = nn.Conv2d(n_feat,n_feat//2,3,1,1)
        self.conv2 = nn.Conv2d(n_feat//2,1,1)
        self.sig = nn.Sigmoid()
    def forward(self, x):
        res = self.block(x)
...
class VIT2Decoder(nn.Module):
    ...
    def forward(self, feats):
        ...
        # exit()
        op[("disp", 0)] = self.block1(out_dec_level1)
        op[("disp", 1)] = self.block2(out_dec_level2)
        op[("disp", 2)] = self.block3(out_dec_level3)
        op[("disp", 3)] = (self.block3(F.interpolate(out_dec_level3, scale_factor = 0.5)))
