...
class Depth_Block(nn.Module):
    ...
    def forward(self, x, up = False):
        ...
        res = self.block(x)
        if up:
            res = self.sig(self.conv2(self.up(self.conv1(res))))
        else:
            res = self.sig(self.conv2(self.conv1(res)))
        return res
#############################################
class VIT2Encoder_light(nn.Module):
    def __init__(self, 
        ...
        self.conv_init = nn.Sequential(nn.Conv2d(12,32,3,1,1), nn.Conv2d(32,32,3,1,1), nn.Conv2d(32,32,3,1,1), nn.Conv2d(32,32,3,1,1))
    def forward(self, inp_img):
        inp_img = self.down_img(inp_img)
        # print(inp_img.size())
        # exit()
...
class VIT2Decoder_light(nn.Module):
    ...
    def forward(self, feats):
        ...
        # exit()
        op[("disp", 0)] = self.block1(out_dec_level1, True)
        op[("disp", 1)] = self.block2(out_dec_level2, True)
        op[("disp", 2)] = self.block3(out_dec_level3, True)
        op[("disp", 3)] = self.block3(out_dec_level3)
